    ...
    navigator.geolocation.getCurrentPosition(
      async (pos) => {
        try {
          ...
          const res = await axios.get(`/api/weather/coords?lat=${lat}&lon=${lon}`)
          setWeather(res.data)
    ...
    return (
      <div className="aira-panel p-4">
        ...
        <div className="flex items-center justify-center gap-2 h-20">
          ...
          <span className="text-xs text-aira-text-dim font-mono">
            {locating ? 'LOCATING…' : 'LOADING…'}
          </span>
        </div>
      </div>
    )
  ...
  return (
    <div className="aira-panel p-4">
      <div className="flex items-center justify-between mb-3">
        ...
        <div className="flex items-center gap-2">
          <button onClick={detectLocation} title="Use my location" className="text-aira-text-dim hover:text-aira-blue transition-colors">
            <LocateFixed className="w-3.5 h-3.5" />
          </button>
          <button onClick={() => fetchWeather(city || 'London')} className="text-aira-text-dim hover:text-aira-blue transition-colors">
          ...
          <div className="flex items-start justify-between">
            <div>
              ...
              <div className="flex items-center gap-1 mt-0.5">
                ...
                <span className="text-xs text-aira-text-dim">{current.city}</span>
              </div>
            </div>
